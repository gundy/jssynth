...
import {createBlankSong, Song} from '../Song'
...
export class MODLoader implements Loader {
  loadSong(data: ArrayBuffer): Song {
    ...
    let readWord = function (ofs) {
      return (bytes[ofs] * 256 + bytes[ofs + 1] );
    };
    let modType:string = bytesToLatin1(bytes, 1080, 1084);
    let modTypeData = MODTypes[modType] || { key: 'NOIS', channels: 4, instruments: 15 };
    let song:Song = createBlankSong();

    song.name = bytesToLatin1(bytes, 0, 20);
    ...
    song.songLength = bytes[songLengthPos];
    song.orders = [];
    let maxPatternNum = 0;
    for (i = 0; i < 128; i++) {
      song.orders[i] = bytes[songLengthPos + 2 + i];
    ...
    for (i = 0; i <= maxPatternNum; i++) {
      ...
      for (row = 0; row < 64; row++) {
        ...
        for (chan = 0; chan < modTypeData.channels; chan++) {
          ...
          let b1 = bytes[chanOfs];
          let b2 = bytes[chanOfs + 1];
          let b3 = bytes[chanOfs + 2];
          let b4 = bytes[chanOfs + 3];
    ...
    (function() {
      ...
      for (i = 0; i < modTypeData.instruments; i++) {
        ...
        sampleName = bytesToLatin1(bytes, insOffset, insOffset + 22);
        ...
        sample = new Sample(bytes, {
          name: sampleName,
          bits: 8,
          channels: 1,
          signed: true,
          sampleRate: 44100,
          representedFreq: 44100 / Math.pow(EIGHTH_SEMITONE_MULTIPLIER, MOD_FINETUNE_TABLE[bytes[insOffset + 24]]),
          sampleLength: sampleLength,
          volume: bytes[insOffset + 25],
